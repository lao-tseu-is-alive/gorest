...
// inspired from https://gist.github.com/creack/4c00ee404f2d7bd5983382cc93af5147
import (
	"context"
	"flag"
	"fmt"
...
func main() {
	...
	flag.StringVar(&listenAddr, "listen-addr", adr, "server listen address")
	/*
		flag.BoolVar(&disableLog, "log-disabled", true, "disable logging feature")
		flag.BoolVar(&disableRequestID, "request-id-disabled", true, "disable request id feature")
	...
	*/
	...
	server := &http.Server{
		Addr:         listenAddr,
		Handler:      (middlewares{c.tracing}).apply(router),
		ErrorLog:     logger,
...
func (c *controller) index(w http.ResponseWriter, req *http.Request) {
	...
	w.Write(HelloWorldBytes) // writing bytes is more efficient
	//fmt.Fprintln(w, HelloWorldString)
}
